...
export const js = async (file: string): Promise<Page> => {
  /**
  ...
  const url = parse.url(module.url) ?? `${path.posix.join(path.parse(file).dir, slugify(title))}/`;
...
export const md = async (file: string): Promise<Page> => {
  const [raw, stat] = await Promise.all([
  ...
  const url = parse.url(metadata.url) ?? `${path.posix.join(path.parse(file).dir, slugify(title))}/`;
